...
start-pods: clean-pods
	kubectl run elasticsearch --image docker.elastic.co/elasticsearch/elasticsearch:8.0.0 --port "9200" --expose --env "cluster.name=test" --env "discovery.type=single-node" --env "ELASTIC_PASSWORD=changeme" --env "xpack.security.enabled=true" --env "ES_JAVA_OPTS=-Xms512m -Xmx512m" --env "path.repo=/tmp" --limits "cpu=500m,memory=1024Mi"
...
local-build:
	mkdir -p registry/registry.terraform.io/co42/elasticsearch/1.0.0/linux_amd64
	go build -o registry/registry.terraform.io/co42/elasticsearch/1.0.0/linux_amd64/terraform-provider-elasticsearch
